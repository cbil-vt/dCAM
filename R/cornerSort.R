...
#' @importFrom rJava .jnew .jcall .jarray .jevalArray
#' @export
#' @examples
...
cornerSort <- function(X, K, nComb){
    corner.detect <- rJava::.jnew("CornerDetectTopN", rJava::.jarray(X, dispatch=TRUE),
                            as.integer(K), as.integer(nComb))
    rJava::.jcall(corner.detect, "Z", "search")
    idx <- rJava::.jcall(corner.detect, "[[I", "getTopNConv")
    idx <- vapply(idx, .jevalArray, integer(K))
    error <- rJava::.jcall(corner.detect, "[D", "getTopNConvErr")
